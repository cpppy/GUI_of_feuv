...
from pmds import mdsconnect,mdsopen,mdsvalue

class mywindow(gtk.Window):
       def __init__(self):
             super(mywindow,self).__init__()

             self.set_title("fEUVplot")
             ...
             numprebut=gtk.Button('Previous shot')
             numprebut.set_size_request(120,30)
             fixed.put(numprebut,350,100)             
             numprebut.connect('clicked',self.OnNumPreBut,'Previous shot')

             numnexbut=gtk.Button('Next shot')
             numnexbut.set_size_request(120,30)
             fixed.put(numnexbut,500,100)
             numnexbut.connect('clicked',self.OnNumNexBut,'Next shot')
             ...
             global time
             self.timetext.set_text('4.0000')
             time=self.timetext.get_text()
             self.timetext.connect('key-release-event',self.on_time_release)

             timelabel2=gtk.Label('(s)')
             timelabel2.set_size_request(30,30)
             fixed.put(timelabel2,280,200)
